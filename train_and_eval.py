...
from model import vgg16bn, only_conv_layers_model, only_dense_model, load_train_conv_output, load_valid_conv_output, save_conv_output, linear_model
import numpy as np
import pdb
...
model = linear_model(settings.training_classes)
model.summary()
model.fit_generator(train_gen, settings.training_batch_count, epochs=10, validation_data = val_gen, validation_steps=settings.validation_batch_count, callbacks=settings.callbacks)
